...
/* for My changing time flow Hypercalls */
unsigned int vcpu_slope_force = 0;  /* The bit is used for hypercall do_time_slope and do_disable_time_slope. */
int vcpu_tsc_slope = 0;  /* The bit is the slope of vcpu's tsc. */
...
/*
 * functions for changing the speed of time flow
 */

int force_vcpu_tsc_slope(int slope)
{
    /*
    printk("Slope in time.c is : %d\n", slope);
    */
    vcpu_tsc_slope = slope;
    vcpu_slope_force = 1;
    return 0;
}

int disable_vcpu_tsc_slope(void)
{
    vcpu_slope_force = 0;  /* still need to add some RCUs */
    vcpu_tsc_slope = 0;

    if(vcpu_slope_force == 0 && vcpu_tsc_slope == 0)	
	printk("The time slope part in time.c has been disabled!\n");
