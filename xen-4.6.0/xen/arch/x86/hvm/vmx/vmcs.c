...
static int vmx_init_vmcs_config(void)
{
    ...
    if ( _vmx_secondary_exec_control & SECONDARY_EXEC_ENABLE_VM_FUNCTIONS )
    {
		...
		printk("eptp_switching support? %llu\n", _vmx_vmfunc & VMX_VMFUNC_EPTP_SWITCHING);
    }
    ...
    if ( !vmx_pin_based_exec_control )
    {
		...
		printk("vmx_secondary_exec_control = %lx\n", (unsigned long)vmx_secondary_exec_control);
...
static int construct_vmcs(struct vcpu *v)
{
	...
	printk("WTF! vcpu %d's exec_control = %lx\n", v->vcpu_id, (unsigned long) v->arch.hvm_vmx.exec_control);
...
//                                          CPU_BASED_CR3_LOAD_EXITING |
                                          CPU_BASED_CR3_STORE_EXITING);
		printk("paging mode hap...\n");
		printk("WTF2! vcpu %d's exec_control = %lx\n", v->vcpu_id, (unsigned long) v->arch.hvm_vmx.exec_control);
    }
    else
    {
    ...
    v->arch.hvm_vmx.secondary_exec_control &=
        ~(SECONDARY_EXEC_ENABLE_VM_FUNCTIONS |
          SECONDARY_EXEC_ENABLE_VIRT_EXCEPTIONS);
#endif
	printk("WTF! vcpu %d's secondary_exec_control = %lx\n", v->vcpu_id, (unsigned long) v->arch.hvm_vmx.secondary_exec_control);
