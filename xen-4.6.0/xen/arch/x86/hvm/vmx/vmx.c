...
static void vmx_restore_guest_msrs(struct vcpu *v)
{
    ...
    while ( guest_flags )
    {
		...
		/* lwj */
		/*
		if (msr_index[i] != MSR_IA32_DEBUGCTLMSR){
			printk("%lx\n",(unsigned long)msr_index[i]);
...
static int vmx_load_msr(struct vcpu *v, struct hvm_msr *ctxt)
{
    ...
    {
    	/* for debugging */
		if ( ctxt->msr[i].index == MSR_IA32_DEBUGCTLMSR )
			printk("v=%lld,r=%ld", (unsigned long long)ctxt->msr[i].val, (unsigned long)ctxt->msr[i]._rsvd);
		printk("l");
...
#if 0
static void vmx_set_cr3_load_exiting(struct vcpu *v, bool_t enable)
{
...
static void vmx_enable_msr_exit_interception(struct domain *d)
{
    ...
    for_each_vcpu ( d, v ) {
        for ( i = 0; i < vmx_introspection_force_enabled_msrs_size; i++ ) {
			printk("enabling interception WR for msrs...\n");
            vmx_enable_intercept_for_msr(v, vmx_introspection_force_enabled_msrs[i],
                                         MSR_TYPE_W);
			vmx_enable_intercept_for_msr(v, vmx_introspection_force_enabled_msrs[i],
                                         MSR_TYPE_R);
        }
		/* Enable interception for debug control. */
		printk("enabling interception WR for ia32 debugctlmsr...\n");
		vmx_enable_intercept_for_msr(v, MSR_IA32_DEBUGCTLMSR,
                                         MSR_TYPE_W);
		vmx_enable_intercept_for_msr(v, MSR_IA32_DEBUGCTLMSR,
                                         MSR_TYPE_R);
    }
...
static void vmx_vcpu_update_vmfunc_ve(struct vcpu *v)
{
    ...
    __vmwrite(SECONDARY_VM_EXEC_CONTROL,
              v->arch.hvm_vmx.secondary_exec_control);
	printk("update VMCS for VMFUNC successfully!\n");
...
//    .set_cr3_load_exiting = vmx_set_cr3_load_exiting,
...
static const struct lbr_info {
    ...
    { MSR_IA32_LASTINTTOIP,         1 },
    { MSR_C2_LASTBRANCH_TOS,        1 },
    { MSR_P4_LASTBRANCH_0_FROM_LIP, 32 },
    { MSR_P4_LASTBRANCH_0_TO_LIP,   32 },
    { 0, 0 }
}, at_lbr[] = {
    { MSR_IA32_LASTINTFROMIP,       1 },
    { MSR_IA32_LASTINTTOIP,         1 },
...
static int vmx_msr_read_intercept(unsigned int msr, uint64_t *msr_content)
{
    ...
    case MSR_IA32_SYSENTER_EIP:
		printk("sysenter eip read\n");
        __vmread(GUEST_SYSENTER_EIP, msr_content);
        break;
    case MSR_IA32_DEBUGCTLMSR:
		printk("ia32 debugctl msr read\n");
...
static int vmx_msr_write_intercept(unsigned int msr, uint64_t msr_content)
{
	...
	//printk("mwi-debugctl-1: %llx, On cpu %d\n", debugctl,smp_processor_id());
    ...
    case MSR_IA32_DEBUGCTLMSR: {
		...
		/* for debugging */
		printk("ia32 debugctl msr write, c=%lld\n", (unsigned long long)msr_content);
	...
	//rdmsrl(MSR_IA32_DEBUGCTLMSR, debugctl);
	//printk("mwi-debugctl-2: %llx, On cpu %d\n", debugctl, smp_processor_id());
...
static uint64_t intel_pmu_lbr_tos(void) {
	unsigned long long tos;
	rdmsrl(MSR_C2_LASTBRANCH_TOS,tos);
	tos &= 0xF;
	//printk("MSR_START_ADDR 0x%llx\n",tos);
	return tos;
}
...
void _intel_lbr_enable(void) {
	...
	/* a new way - to modify the guest value */
	//__vmwrite(GUEST_IA32_DEBUGCTL, msr_content);
...
void intel_lbr_enable4enter(void* info) {
    unsigned long long debugctl;
...
void _intel_pmu_lbr_disable(void) {
    uint64_t debugctl;
    rdmsrl(MSR_IA32_DEBUGCTLMSR, debugctl);
    debugctl &= ~(IA32_DEBUGCTLMSR_LBR);
    wrmsrl(MSR_IA32_DEBUGCTLMSR, debugctl);
...
int max_length(int* a, int length) {
        ...
        int i;
        for (i=0;i<length; i++) { 
              if (i != 0 && a[i] <= Tg && a[i] >0 && a[i-1] <= Tg && a[i-1] > 0) {
                tmp++;
                if (i == length-1 && tmp > max)
                    max = tmp;
              }
              else if (a[i] > Tg || a[i] <= 0) {
                      if (tmp > max)
                        max = tmp;
                      tmp = 0;
              }else
...
void read_lbr(void* info) {
	...
	lbrentry_t lbrentries[NUM_MSR];
	int Length[NUM_MSR-1] = { 0 };
	/* define for tracing LBR_INFO */
	//int length = 0;
	struct vcpu *v = info;

	for(lbr_pair_i = 0; lbr_pair_i < NUM_MSR; lbr_pair_i++) {
		lbrentries[lbr_pair_i].from = 0;
		lbrentries[lbr_pair_i].to = 0;
	}
	//_intel_pmu_lbr_disable();
	tos = intel_pmu_lbr_tos();
	//printk(KERN_INFO "tos: %X\n",tos);
	for (lbr_pair_i = 0; lbr_pair_i < NUM_MSR; ++lbr_pair_i) {
        unsigned long idx = (tos - lbr_pair_i) & (NUM_MSR - 1);
        //uint64_t from,to;
		msr_from_counter1 = MSR_P4_LASTBRANCH_0_FROM_LIP + idx;
           msr_to_counter1 = MSR_P4_LASTBRANCH_0_TO_LIP  + idx;
           asm volatile    (
                "mov %4, %%ecx;"
                "rdmsr;"
                "mov %%eax, %0;"
        ...
        //rdmsrl(MSR_P4_LASTBRANCH_0_FROM_LIP + idx, from);
        //rdmsrl(MSR_P4_LASTBRANCH_0_TO_LIP + idx, to);
		lbrentries[lbr_pair_i].from = ax1f;
        lbrentries[lbr_pair_i].to = ax1t;

		/* for debugging lbr */
        if (ax1f != 0 && (ax1f <0x81000000 || ax1f > 82000000)) {	
			printk(KERN_INFO "brfrom: %8x (MSR: %X), to %8x (MSR: %X)\n",
ax1f,msr_from_counter1,ax1t,msr_to_counter1);
		}
		

		/* for tracing */
		//HVMTRACE_2D(LBR_INFO, (unsigned long)v->domain->domain_id, (unsigned long)length);
    }
	
	/* for debugging the bit flip */
	//rdmsrl(MSR_IA32_DEBUGCTLMSR, debugctl);
	//printk("deb-ctl:%llx,cpu%d\n", debugctl, smp_processor_id());

	/* length calc */
	for (lbr_pair_diff_i = 0; lbr_pair_diff_i < NUM_MSR-1; ++lbr_pair_diff_i) {
        Length[lbr_pair_diff_i] = lbrentries[lbr_pair_diff_i].from - lbrentries[lbr_pair_diff_i+1].to;
        //if (Length[lbr_pair_diff_i] < 0) 
        //	Length[lbr_pair_diff_i] = -Length[lbr_pair_diff_i];
        /* cannot record TSC here... */
        //HVMTRACE_4D(LBR_INFO, (unsigned long)v->domain->domain_id, Length[lbr_pair_diff_i], 
        //(unsigned long)smp_processor_id(), v->arch.hvm_vcpu.hw_cr[3]);
		//if (Length[lbr_pair_diff_i] < 0xFF && Length[lbr_pair_diff_i] > 0)
        //printk("Length[%d]:0x%x\n", lbr_pair_diff_i, Length[lbr_pair_diff_i]);
	}
	cur_max_length = max_length(Length,sizeof(Length)/sizeof(int));
	//printk("max length:0x%x\n", cur_max_length);
    HVMTRACE_4D(LBR_INFO, (unsigned long)v->domain->domain_id, cur_max_length, 
        (unsigned long)smp_processor_id(), v->arch.hvm_vcpu.guest_cr[3]);
	/* for debugging cr3 */
	//printk("hw_cr3:%lx\n", v->arch.hvm_vcpu.hw_cr[3]);
...
void vmx_vmexit_handler(struct cpu_user_regs *regs)
{
	...
	tos = intel_pmu_lbr_tos();*/
	//on_each_cpu(_intel_pmu_lbr_disable,NULL,1);
	...
	*/
    ...
    case EXIT_REASON_TASK_SWITCH: {
		...
		/* test if there is any task switch... */
		if (source == 0) printk("tsw via call\n");
		else if (source == 1) printk("tsw via iret\n");
		else if (source == 2) printk("tsw via jmp\n");
		else if (source == 3) printk("tsw via gate\n");
		else printk("tsw exit exists\n");
	...
	*/
    ...
    case EXIT_REASON_CR_ACCESS:
    {
		...
		//printk("%lu\n", exit_qualification);
		if ( (exit_qualification & 0x0000000f) == 3) {
		//	printk("p4cr3\n");
			if ( (exit_qualification & 0x00000030) == 0)
			//printk("p4mov to\n");
			{
				/* disable the LBR */
...
/*
				for (lbr_pair_i = 0; lbr_pair_i < NUM_MSR; ++lbr_pair_i) {
        			...
        			rdmsrl(MSR_P4_LASTBRANCH_0_FROM_LIP + idx, from);
        			rdmsrl(MSR_P4_LASTBRANCH_0_TO_LIP + idx, to);
					lbrentries[lbr_pair_i].from = from;
        			lbrentries[lbr_pair_i].to = to;
        			//printk("FROM 0x%p", from);
        			//printk("TO   0x%p", to);
    			}
				for (lbr_pair_diff_i = 0; lbr_pair_diff_i < NUM_MSR-1; ++lbr_pair_diff_i) {
        			Length[lbr_pair_diff_i] = lbrentries[lbr_pair_diff_i].from - lbrentries[lbr_pair_diff_i+1].to;
        			if (Length[lbr_pair_diff_i] < 0) 
           	 			Length[lbr_pair_diff_i] = -Length[lbr_pair_diff_i];
					if (Length[lbr_pair_diff_i] < 0xFF && Length[lbr_pair_diff_i] > 0)
        				printk("Length [%d]:0x%lx\n", lbr_pair_diff_i, Length[lbr_pair_diff_i]); 
    			}
*/

				cpu = current->arch.hvm_vmx.active_cpu;
				on_selected_cpus(cpumask_of(cpu), read_lbr, current, 1);

				/* enable the LBR */
				_intel_lbr_enable();
				__vmwrite(GUEST_IA32_DEBUGCTL, 1);
				
			}
			if ( (exit_qualification & 0x00000030) == 1)
				printk("p4mov from\n");
		}
...
out:
	...
	//on_each_cpu(intel_lbr_enable4enter,NULL,1);
...
void vmx_vmenter_helper(const struct cpu_user_regs *regs)
{
	...
	/* enable before vm enter */	
	//if ( cpu != -1 )
		//on_selected_cpus(cpumask_of(cpu), intel_lbr_enable4enter, NULL, 1);	
	//on_each_cpu(intel_lbr_enable4enter,NULL,1);
	/*
	rdmsrl(MSR_IA32_DEBUGCTLMSR, debugctl);
	printk("%llx,c%d\n", debugctl,smp_processor_id());
