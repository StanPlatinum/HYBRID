...
/* for time_slope */
#define CYCLE_PER_SECOND 3600000000
unsigned int hypercall_force = 0;  /* The bit is used to determine whether or not extra hypercalls are called. */

#define MAX_SLOPE_OPTION 10
unsigned int rdtsc_slope = 0;  /* The number is used to change RDTSC return value in hvm rdtsc intercept. */
unsigned long long tsc_offset_constant = 0;  /* for test */
int rdtsc_slope_flag = 0;  /* The bit is used to mark the rdtsc_slope is positive or negative. */

/* for debugging */
#define BUF_SIZE 0x2010
struct comport_log_t {
    //char dbg_info;
    long dbg_data;
}comportlog[BUF_SIZE];
int comportlogptr = 0;

#define PRINT_INTERVAL 0x400
static void print_comportflow_log(void) 
{
    int i;
    printk("\n%s\n", __FUNCTION__);
    for ( i = 0; i < comportlogptr; i++){
	if (i % PRINT_INTERVAL == 0) {
    	//printk("-%c", comportlog[i].dbg_info);
	printk("-%ld", comportlog[i].dbg_data);
	}
    }
    comportlogptr = 0;
}
#if 0
static void add_comportflow_log4char(char dbg_message) 
{
    comportlog[comportlogptr].dbg_info = dbg_message;
    comportlogptr++;
    if (comportlogptr > BUF_SIZE - 16) 
	print_comportflow_log();
}
#endif
static void add_comportflow_log4long(long dbg_message)
{
    comportlog[comportlogptr].dbg_data = dbg_message;
    comportlogptr++;
    if (comportlogptr > BUF_SIZE - 16) 
	print_comportflow_log();
}
...
/* for EPTP-list test*/
#define EPTP_LIST_SIZE 0x200
unsigned long long eptp_list[EPTP_LIST_SIZE];
int eptp_idx = 0;

/* for rdtsc emulation scaling */
int scale4rdtsc = 64;
/* 64 is a safe number */
...
/* for keeping track of every VM's n (number_of_bits) */
#define MAX_DOMU_NUM 8	/* default to be 8. this max value can be modified in field 'max_loop' in dom0's grub file */
struct pair4guestvm_t {
    unsigned short dom_id;
    int n;
}pair4guestvm[MAX_DOMU_NUM];	
/* length of current struct array */
int s_end = 0;
...
static int __init hvm_enable(void)
{
	...
	printk("Get param, altp2m supported!\n");
    printk("opt_altp2m? %d, supported? %d\n", opt_altp2m_enabled, hvm_funcs.altp2m_supported);
    /* test the domain to see if the altp2m is supported... */
...
void hvm_set_rdtsc_exiting(struct domain *d, bool_t enable)
{
	...
	/*
	printk("Executing hvm_funcs.set_rdtsc_exiting... vcpu_id = %d, enable = %d\n", v->vcpu_id, enable);
	*/
    }
}

#if 0
void hvm_enable_msr_exit_interception(struct domain *d)
{
    hvm_funcs.enable_msr_exit_interception(d);
	printk("Setting msr for introspection... domain = %d\n", d->domain_id);
}

void hvm_set_cr3_load_exiting(struct domain *d, bool_t enable)
{
    struct vcpu *v;

    for_each_vcpu ( d, v ) {
        hvm_funcs.set_cr3_load_exiting(v, enable);
	
	printk("Executing set_cr3_load_exiting... vcpu_id = %d, enable = %d\n", v->vcpu_id, enable);
...
int hvm_domain_initialise(struct domain *d)
{
    ...
    /* inserting a pair when a VM is created */
    printk("***idomainid: %d\n", (int)d->domain_id);
    if (s_end < MAX_DOMU_NUM) {
	pair4guestvm[s_end++].dom_id = d->domain_id;
	printk("inserting done\n");
    }
...
void hvm_domain_destroy(struct domain *d)
{
    ...
    /* deleting pair when a VM is destroyed */
    printk("***ddomainid: %lu\n",(unsigned long) d->domain_id);
    for (i = 0; i < s_end; i++){
	if (pair4guestvm[i].dom_id == d->domain_id) break;
    }
    if (i == s_end) printk("something wrong with the pair when being destroyed!\n");
    else {
	s_end--;
	for (;i < s_end; i++) {
	    pair4guestvm[i].dom_id = pair4guestvm[i+1].dom_id;
	    pair4guestvm[i].n = pair4guestvm[i+1].n;
	}
...
void intel_lbr_enable4hvm(void* info) {
...
static int hvm_load_cpu_msrs(struct domain *d, hvm_domain_context_t *h)
{
	...
	//int cpu = curr->arch.hvm_vmx.active_cpu;
	...
	printk("l");
	/* lwj: flip the bit again! */
	/*
	if ( cpu != -1 )
		on_selected_cpus(cpumask_of(cpu), intel_lbr_enable4hvm, NULL, 1);
	
	on_each_cpu(intel_lbr_enable4hvm, NULL, 1);
	*/
...
#if 0
int id_exist(int curr_domain_id)
{
    int i = 0;	/* loop variable */
    for (i = 0; i < MAX_DOMU_NUM; i++)
	if (pair4guestvm[i].dom_id == curr_domain_id) return i;
    return -1;
}
#endif
...
void hvm_rdtsc_intercept(struct cpu_user_regs *regs)
{
	...
	if ( (idx = p2m_find_altp2m_by_eptp(curr->domain, eptp)) ==
                 INVALID_ALTP2M )
	{
	    printk("Error! EPTP not found in alternate p2m list\n");
	} else {
	    add_comportflow_log4long(idx);
	}
	*/
	/* obtaining the eptp index */
	eptp_idx = vcpu_altp2m(curr).p2midx;
	number_of_bits = eptp_idx * scale4rdtsc;
    }

    /* sorting out the max n */
    x = 0;
    for (j = 0; j < s_end; j++) {
	if (pair4guestvm[j].dom_id == curr->domain->domain_id) 
	    pair4guestvm[j].n = number_of_bits;
	if (pair4guestvm[j].n > x) x = pair4guestvm[j].n;
    }

    /* do the coarsening */
    if (x < 64) {
	add_comportflow_log4long(x);
	tsc = tsc >> x;
	/* make sure the tsc value increases monotonically */
	if (tsc <= tsc_last)
	    tsc++;
	tsc = tsc << x;
    }

	/* adding physical delay */
	//udelay(1);

#if 0
    if (number_of_bits < 64) {
	add_comportflow_log4long(number_of_bits);
	tsc = tsc + (tsc >> number_of_bits);
    }
    /* changing the speed of guest tsc value */
    if ( rdtsc_slope_flag == 1 )
	/* tsc = tsc + (tsc >> rdtsc_slope); */
	tsc = tsc + tsc_offset_constant;
    if ( rdtsc_slope_flag == -1 )
	/* tentative, to change */
	tsc = tsc + tsc_offset_constant;
    if (number_of_bits < 64) {
	add_comportflow_log4long(number_of_bits);
	tsc = tsc + (tsc & (0xffffffffffffffff >> number_of_bits));	
    }	/* multiply the last few bits */
#endif
                            ...
                            bool_t may_defer)
{
    ...
    default:
        if ( (ret = vmce_wrmsr(msr, msr_content)) < 0 )
            goto gp_fault;
        /* If ret == 0 then this is not an MCE MSR, see other MSRs. */
		/* lwj using vmx_msr_write_intercept */
        ret = ((ret == 0)
...
static hvm_hypercall_t *const hvm_hypercall64_table[NR_hypercalls] = {
    ...
    HYPERCALL(intercept_rdtsc),
    HYPERCALL(disable_intercept_rdtsc),
    HYPERCALL(intercept_msr),
...
#if 0    
    if ( (u32)(regs->eax) == 43 ||(u32)(regs->eax) == 44 || (u32)(regs->eax) == 45 )
    {
	printk("111\n");
	printk("mode = %d\n", mode);
    }
    ...
    XEN_GUEST_HANDLE_PARAM(void) arg)
{
	...
	printk("doing altp2m op...\n");

    if ( !hvm_altp2m_supported() )
        return -EOPNOTSUPP;

	printk("preparing altp2m op params...\n");

    if ( copy_from_guest(&a, arg, 1) )
        return -EFAULT;

	printk("parsing the params and checking cmds...\n");

	printk("pad1 = %d, pad2 = %d, version = %d, cmd = %d\n", a.pad1, a.pad2, a.version, a.cmd);
	...
	printk("proceed ony if d is a hvm!\n");
	...
	printk("first we need to activate d's altp2m...\n");
	...
	*/
	...
	printk("but if CPU does not support #VE?\n");
	/*
	...
	*/
	...
	printk("do update VMFUNC altp2m successfully!\n");
        break;
    }

    case HVMOP_altp2m_create_p2m:
        if ( !(rc = p2m_init_next_altp2m(d, &a.u.view.view)) ) {
			printk("do create altp2m successfully!\n");
            rc = __copy_to_guest(arg, &a, 1) ? -EFAULT : 0;
        	}
        break;
...
/*
 * My hypercall for change the speed of time flow
 */

/*
unsigned long vcpu_read_eptp(struct vcpu *v)
{
    unsigned long p;
    vmx_vmcs_enter(v);
    __vmread(EPT_POINTER, &p);
    vmx_vmcs_exit(v);
    return p;
}
void hvm_print_eptp(struct domain *d)
{
    unsigned long p;
    struct vcpu *v;
    for_each_vcpu ( d, v ) {
...
/* functional part */
int do_intercept_rdtsc(int scale)
{
    struct domain *d;
    /*
    int rc4percpu, rc4vpt;
    printk("Intercepting RDTSC and ready to start return virtual_time...\n");   
    rc4percpu = force_vcpu_tsc_slope(slope);
    rc4vpt = force_hvm_guest_time_slope(slope);  
    printk("invoking force_vcpu_tsc_slope\n");
    if (slope > 0) {
	rdtsc_slope = slope;
	rdtsc_slope_flag = 1;
    } else if (slope < 0){
    ...
    printk("rdtsc_slope = %d\n", rdtsc_slope);
    printk("rdtsc_slope_flag = %d\n", rdtsc_slope_flag);
    printk("tsc_offset_constant = %llu\n", tsc_offset_constant);
    */	    	
    printk("Setting 'EXIT_RDTSC' 1\n");
    //add_comportflow_log4char('1');

    scale4rdtsc = scale;
    printk("scale = %d\n", scale4rdtsc);

    for_each_domain ( d ) {
        if ( is_hvm_domain(d) && ~(d->arch.vtsc) ) {
	    printk("Setting domain %d 's VMEXIT_RDTSC\n", d->domain_id);
...
int do_disable_intercept_rdtsc(void)
{
    struct domain *d;    
    /*
    int rc4percpu, rc4vpt;   
    printk("Intercepting RDTSC disabling...\n");
    rc4percpu = disable_vcpu_tsc_slope();
    rc4vpt = disable_hvm_guest_time_slope();
    if (rc4percpu == 0 && rc4vpt ==0) {
	printk("invoking disable_vcpu_tsc_slope\n");
    } else {
    ...
    */
    rdtsc_slope = 0;
    rdtsc_slope_flag = 0;
    tsc_offset_constant = 0;  /* for test */

    scale4rdtsc = 1;
    eptp_idx = 0;

    printk("\nSetting 'EXIT_RDTSC' 0\n");
    //add_comportflow_log4char('0');   

    for_each_domain ( d ) {
        if ( is_hvm_domain(d) && ~(d->arch.vtsc) ) {
		printk("Setting domain %d 's VMEXIT_RDTSC\n", d->domain_id);
		hvm_set_rdtsc_exiting(d, 0);
	}
    }
    return 0;
}

int do_intercept_msr(void)
{
	...
	for_each_domain ( d ) {
		...
		if ( hvm_enable_msr_exit_interception(d) )
			printk("Domain%d's MSR_EIP set successfully\n", d->domain_id);
		}
	}
	return 0;
}
